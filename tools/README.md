...
const DAILY_PLAN = {
    ...
    dayStartsAtHour: 12,       // 中午才換成新的一天（老師下午上課）
    skipDates: ['2026-09-25', '2026-10-09', '2027-01-01'],   // 放假的平日，純標示用
};
```

### `dayStartsAtHour`：幾點換成新的一天

老師**下午**上 ESL 課才教當天那兩個新字。若照午夜換日，早上出門前打開會看到還沒教的字，
順序整個反過來。所以「一天」是從 `dayStartsAtHour` 算起：

| 時間 | 顯示 |
|---|---|
| 9/1 07:30（上學前） | W1 **Day1**（昨天教的，正好複習） |
| 9/1 11:59 | W1 Day1 |
| 9/1 12:00（放學後／課後） | W1 **Day2**（今天新教的） |
| 9/2 08:00（隔天早上） | W1 Day2（仍是昨天的） |

實作是 `studyDate()`：把現在時間往前推 `dayStartsAtHour` 小時，再取那天的日期。
**進度、金幣、連續天數也全部走這個「學習日」**（`studyDateISO()`），不然畫面顯示的日子跟
記錄的日子會對不上。程式裡不要直接用 `new Date()` 判斷今天。

ESL 課時間有變就改這個數字；設 `0` 等於恢復成午夜換日。
